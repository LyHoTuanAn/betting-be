...
describe('GoldZone API with MySQL',()=>{
 it('registers with 0 starting gold and authenticates',async()=>{const response=await request(app).post('/api/auth/register').send({username:testUsername,displayName:'Integration Player',password:'secret123'}).expect(201);token=response.body.token;refreshToken=response.body.refreshToken;userId=response.body.user.id;expect(response.body.user.balance).toBe(0);});
 ...
 it('processes one bet only when idempotency key repeats',async()=>{await prisma.user.update({where:{id:userId},data:{balance:50_000n}});const before=(await request(app).get('/api/me').set('Authorization',`Bearer ${token}`)).body.user.balance,key='same_request_123';const first=await request(app).post('/api/games/slot/spin').set('Authorization',`Bearer ${token}`).set('X-Idempotency-Key',key).send({bet:1000}).expect(200);const second=await request(app).post('/api/games/slot/spin').set('Authorization',`Bearer ${token}`).set('X-Idempotency-Key',key).send({bet:1000}).expect(200);expect(second.body.roundId).toBe(first.body.roundId);expect(await prisma.gameRound.count({where:{userId,requestId:key}})).toBe(1);expect(second.body.balance).toBe(first.body.balance);expect(first.body.balance).toBeGreaterThanOrEqual(before-1000)});
 it('holds withdrawal balance and locks funds properly',async()=>{await prisma.user.update({where:{id:userId},data:{balance:100_000n}});const me=(await request(app).get('/api/wallet').set('Authorization',`Bearer ${token}`)).body;await request(app).post('/api/wallet/withdraw').set('Authorization',`Bearer ${token}`).send({amount:50_000,bankName:'Timo',accountNumber:'123456',accountName:'Test Player'}).expect(201);const after=(await request(app).get('/api/wallet').set('Authorization',`Bearer ${token}`)).body;expect(after.lockedBalance).toBe(50_000);expect(after.availableBalance).toBe(me.availableBalance-50_000);});
});
